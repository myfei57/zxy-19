...
// Freeze durably records the overheat evidence first and only then freezes
// the batch, so a frozen batch always has an overheat record behind it.
...
// status. The flag is written before the status so a crash never leaves a
// frozen batch without its flag.
